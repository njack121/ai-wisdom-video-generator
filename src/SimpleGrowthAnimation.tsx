...
export const SimpleGrowthAnimation: React.FC = () => {
  ...
  // Longer, smoother stages with overlapping transitions
  ...
  // Get smooth progress within each stage
  let stageProgress = 0;
  if (stage === 'seed') {
  ...
  // Continuous growth - all elements are always rendered, just at different stages
  const renderPlant = () => {
    const baseY = 400; // Ground level

    // Calculate continuous values that build on each other
    ...
    return (
      <g>
        ...
        {/* Main stem/trunk - grows continuously */}
        {finalHeight > 0 && (
          <line 
            x1="400" 
            y1={baseY} 
        ...
        {crownSize > 0 && (
          <>
            ...
            {/* Small detail branches in crown */}
            {progress > 0.9 && (
              <>
